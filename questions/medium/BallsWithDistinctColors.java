...
class Solution {
    public int[] queryResults(int limit, int[][] queries) {
        ...
        Map<Integer, Integer> ballColor = new HashMap<>(); // Stores the current color of each ball
        ...
        for (int i = 0; i < n; i++) {
            ...
            // If the ball has been painted before, remove its previous color
            if (ballColor.containsKey(ball)) {
                int prevColor = ballColor.get(ball);
                colorCount.put(prevColor, colorCount.get(prevColor) - 1);

                // If no balls have the previous color, remove it from tracking
            ...
            // If this color was not previously in use, increment distinctColors
